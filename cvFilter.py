...
#                v_flag = False
        ...
        if c_flag and v_flag:
            print('第0天 成交量:[%d]  ' % v_data[-1* LOOKBACKDAYS ])
            print('近3天 成交量:[%d] [%d] [%d]: ' % (v_data[1],v_data[2],v_data[3]))
            print(data)
...
print rsiSet
print "综上，符合所有： %d" %len(list(positiveSet & macdSet & dmiSet & dcSet & rsiSet))
print positiveSet & macdSet & dmiSet & dcSet & rsiSet
################################
...
if len(targetDict):
    msgText = '今日共发现{}个股票{}满足量价条件，技术指标筛选如下：\n'.format(len(targetDict), targetDict.keys()) + \
          ...
          '[4]突破唐奇安通道: {} \n'.format(list(dcSet)) + \
          '[5]RSI指标3天上涨: {} \n'.format(list(rsiSet)) + \
          '符合所有规则有{}个 \n'.format(len(list(positiveSet & macdSet & dmiSet & dcSet & rsiSet)))
    ...
    msgHtml = '<html><body><div><div>今日共发现<b><font color="#ff0000">{}</font></b>个股票<b><font color="#ff0000">{}</font></b>满足量价条件，技术指标筛选如下：</div>'.format(
        len(targetDict),targetDict.keys()) \
              ...
              + '<div><b>[5]RSI指标3天上涨:</b> {}&nbsp;</div>'.format(list(rsiSet)) \
              + '<div>符合所有规则有{}个, 近{}个交易日走势如下&nbsp;</div></div>'.format(len(list(positiveSet & macdSet & dmiSet & rsiSet)),K_Length) \
