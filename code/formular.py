...
#    print LL
    ...
    var = np.zeros(z)
#    print "MAX(CLOSE - LC, 0)"
    MAX0=MAX(LL, var)
    
    for i in range (0,z):
        var[i] = MAX0[i]
#    print "MAX(CLOSE - LC, 0) len=%d"%z
#    print var
    
    RSI1=SMA(var, N1, 1) / SMA(ABS(CLOSE - LC), N1, 1) * 100
    RSI2=SMA(var, N2, 1) / SMA(ABS(CLOSE - LC), N2, 1) * 100
    RSI3=SMA(var, N3, 1) / SMA(ABS(CLOSE - LC), N3, 1) * 100
...
def RSIRaise(code,N=6):
    
    last = datetime.datetime.today().strftime('%Y-%m-%d')
    first,last = lookback_trade_cal(last,30)

    DF =ts.get_k_data(code, start=first, end=last)
    ...
    data_index = 1
    while data_index < 4:
        rsi_now=rsiset.loc[lenrsi-data_index,['RSI1']]   
        rsi_old=rsiset.loc[lenrsi-data_index-1,['RSI1']]
        #print rsi_now.values, rsi_old.values
        if rsi_now.values >= rsi_old.values:
            data_index += 1
        else:
            v_flag = False
            break   
...
if __name__ == '__main__':
    ...
    #a =ts.get_k_data('600848', start='2017-12-05', end='2017-12-08')
    #a =ts.get_k_data('000503')    
    #print a
    #print CCI(a).tail()
    RSIRaise('000503')
