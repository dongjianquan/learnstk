...
#                ax.axhline(entry_price[i], xmin = 1 - self.delta_days * 1.0 /self.back_days,color="y", linestyle="-.")
            ...
            #xs = data['date']
            #ax.xaxis.set_major_formatter(datetime.DateFormatter('%Y-%m-%d'))#设置时间标签显示格式
           ...
           # ax.gca().xaxis.set_major_locator(mdates.DayLocator())
            
            ax.plot(data['d_up'], color='r', label='Up: {} days'.format(self.D_Channel['up']))
            ax.plot(data['d_down'], color='b',label='Down: {} days'.format(self.D_Channel['down']))
            ax.legend(loc = 0)
            ax.set_title(code)
            ax.grid(True)
            ...
            # 绘制ATR图
            ax4.plot(data['atr'], color='r', label='atr')
            ax4.legend(loc = 0)
