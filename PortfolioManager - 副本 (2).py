...
class PortfolioManger:
    ...
    def __init__(self):
        ...
        for record in tmp:
            self.portfolio[record['code']] = {'entry_price': record["entry_price"], 'entry_date': record["entry_date"], \
    ...
    def getData(self):
        ...
        for code, value in self.portfolio.items():
            ...
            print self.stock_data[code]['date']
    ...
    def createFig(self):
        ...
        for code, data in self.stock_data.items():
            ...
            print "----------"
            xs = [datetime.datetime.strptime(d, '%Y-%m-%d').date() for d in entry_date]
            entry_date = xs
            ...
            # 绘制唐奇安通道
            xs = [datetime.datetime.strptime(d, '%Y-%m-%d').date() for d in data['date']]
            #xs = [datetime.date.fromtimestamp(d) for d in data['date']]
            print date2num(xs)
            
            ax.plot(date2num(xs), data['d_up'], color='r', label='Up: {} days'.format(self.D_Channel['up']))
            ax.plot(date2num(xs), data['d_down'], color='b',label='Down: {} days'.format(self.D_Channel['down']))
            ax.legend(loc = 0)
            ax.set_title(code)
            ax.grid(True)
            ...
            # 绘制ATR图
            ax4.plot(date2num(xs), data['atr'], color='r', label='atr')
            ax4.legend(loc = 0)
            ax4.set_xticks(range(0, len(xs), 5))
            ax4.set_xticklabels(xs[::5], rotation=45)
            ax4.grid(True)
